var VueAppConfiguration = {
    ...
    created() {
        var i;
        var item;
        var itemImagesRoot = 'images/';
        for (i = 0; i < this.items.length; i++) {
            item = this.items[i];
            item.isSelected = false;
            item.isDetailsVisible = false;
            item.orderQty = 1;
            item.imgSrc = itemImagesRoot + item.imgSrc;
    ...
    beforeUpdate() {
        var i;
        var item;
        this.totalCostOfSelectedItems = 0;
        for (i = 0; i < this.items.length; i++) {
            item = this.items[i];
            if (item.isSelected === true) {
                this.totalCostOfSelectedItems += item.cost * item.orderQty;
            }
        }
    },
    methods: {
